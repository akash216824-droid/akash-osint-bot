...
def is_collaborator(user_id):
    return user_id in load_access().get("collaborators", [])

def is_moderator(user_id):
    return user_id in load_access().get("moderators", [])

def has_special_access(user_id):
    return is_collaborator(user_id) or is_moderator(user_id)
...
async def is_member(user_id, context):
    """सख्त verification – हर चैनल/ग्रुप में मेंबर होना जरूरी।"""
    for ch in CHANNELS:
        # Skip private group (invite link)
    ...
    for record in results:
        ...
        for k, v in record.items():
            if v is not None and v != "" and v != "N/A":
                clean_record[k] = v
        # Ensure email is in every record
...
def format_tg_to_num_output(data):
    ...
    country = data.get("Country"); country_code = data.get("Country Code")
    if not phone: return "❌ No data found for this Telegram ID."
    clean_data = {"Telegram ID": tg_id, "Phone": phone, "Country": country or "N/A",
                  "Country Code": country_code or "N/A", "developer": "𐙚 𓆩𝘼𝙠𝙖𝙨𝗵 𝙊𝙨𝙞𝙣𝙩𓆪𓂃🧑💻🎀⃤"}
    return "**TG to Num Lookup**\n```json\n" + json.dumps(clean_data, indent=4, ensure_ascii=False) + "\n```"

def format_ifsc_output(data):
    if not data: return "❌ No data found."
    if "success" in data and data["success"] == False: return "❌ Invalid IFSC code."
    ...
    try:
        ...
        if await is_verified(user_id, context):
            ...
            await update.message.reply_text(
                "⚠️ Please join all channels & group to use this bot.\n\n"
...
async def bot_management_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_back":
        ...
        await query.edit_message_text("🚫 **Block User**\n\nSend User ID.", reply_markup=get_back_keyboard())
        ...
        msg = "🚫 **Blocked Users**\n\n" + "\n".join(str(uid) for uid in blocked_set) if blocked_set else "✅ No blocked users."
        await query.edit_message_text(msg, parse_mode="Markdown", reply_markup=get_back_keyboard())
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_verified(user_id, context):
        await update.message.reply_text(
            "⚠️ You must join all channels & group to use this bot.\n\n"
            "After joining, press the Verify button.",
            reply_markup=get_verify_keyboard()
        )
        return
    ...
    if user_id in ADMIN_IDS and context.user_data.get("admin_action"):
        ...
        if action == "giveallcoins":
            if text.isdigit():
                amount = int(text); data = load_data(); count = 0
                for uid in data:
                    data[uid]["coins"] += amount; count += 1
                save_data(data)
        ...
        if action == "block":
            if text.isdigit():
                blocked = load_blocked(); blocked.add(int(text)); save_blocked(blocked)
        ...
        if action == "unblock":
            if text.isdigit():
                blocked = load_blocked(); blocked.discard(int(text)); save_blocked(blocked)
        ...
        if action == "add_collab":
            if text.isdigit():
                target = int(text); access = load_access()
                if target not in access["collaborators"]:
                    ...
                    save_access(access)
                    await update.message.reply_text(f"✅ User `{target}` is now a **Collaborator**.", parse_mode="Markdown")
                else:
                    await update.message.reply_text(f"ℹ️ Already a Collaborator.", parse_mode="Markdown")
            else:
                await update.message.reply_text("❌ Invalid ID.")
            context.user_data.pop("admin_action")
            return
        ...
        if action == "add_mod":
            if text.isdigit():
                ...
                if target not in access["moderators"]:
                    ...
                    save_access(access)
                    await update.message.reply_text(f"✅ User `{target}` is now a **Moderator**.", parse_mode="Markdown")
                else:
                    await update.message.reply_text(f"ℹ️ Already a Moderator.", parse_mode="Markdown")
            else:
                await update.message.reply_text("❌ Invalid ID.")
            context.user_data.pop("admin_action")
            return
        ...
        if action == "remove_access":
            if text.isdigit():
                ...
                if removed:
                    ...
                    await update.message.reply_text(f"ℹ️ No special access.", parse_mode="Markdown")
            else:
                await update.message.reply_text("❌ Invalid ID.")
            context.user_data.pop("admin_action")
            return
...
def main():
    ...
    # ---------- FIXED: Timeout + Reliable Polling ----------
    application = (
        Application.builder()
        .token(BOT_TOKEN)
        .read_timeout(30)
    ...
    # Callbacks (patterns are precise)
    ...
    application.run_polling(
        poll_interval=1.0,
        timeout=30,
        drop_pending_updates=True   # पुराने pending updates skip करें
    )
